...
void flipled(int x, int y, int z)
{
  cube[z][y] ^= (1 << x);
}
...
// plane is of value "x", "y" or "z"
void setplane(byte plane, int i, int state)
{
  if (plane == 'x')
  {
    if (state > 0)
...
// byte anchor is the plane that is constant. ("x","y", or "z")
void drawline_plane(int x2, int y2, int x1, int y1, byte anchor){
